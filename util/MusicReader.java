  ...
  public static <T> T parseFile(Readable readable, CompositionBuilder<T> piece) {
    ...
    while (scanner.hasNext()) {
      ...
      switch (lineType) {
        case "tempo":
          try {
            piece.setTempo(scanner.nextInt() / 1000 * 1000);
        ...
        case "note":
          ...
          break;
        case "end":
          List<Repeat> ends = new ArrayList<>();
          try {
            while (scanner.hasNextInt()) {
              ends.add(new Repeat(scanner.nextInt(), scanner.nextInt()));
            }
            piece.addMultiEnding(ends);
          } catch (NoSuchElementException e) {
